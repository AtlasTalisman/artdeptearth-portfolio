...
import { projects, TAG_TAXONOMY } from "@/data/projects";
...
/** Parse a dash-separated filter string into valid tag keys */
export function parseFilterTags(filter: string | undefined): string[] {
  if (!filter) return [];
  return filter
    .toLowerCase()
    .split("-")
    .filter((t) => t in TAG_TAXONOMY);
}

export default function HomeContent({ filterTags = [] }: { filterTags?: string[] }) {
  ...
  // Filter + sort: projects matching more active tags rank higher
  const displayProjects = useMemo(() => {
    if (!isFiltered) return projects;
    const scored = projects
      .map((p) => {
        const matchCount = filterTags.filter((t) => p.tags.includes(t)).length;
        return { project: p, matchCount };
      })
      .filter((s) => s.matchCount > 0)
      .sort((a, b) => b.matchCount - a.matchCount);
    return scored.map((s) => s.project);
  }, [filterTags, isFiltered]);
    ...
    <div className="relative w-full min-h-screen bg-white text-black">
      ...
      <section className="relative w-full h-screen overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-6 py-5">
          <a href="/" className="font-mono text-[10px] font-bold tracking-[0.15em] uppercase hover:underline">
            Atlas Talisman
          </a>
      ...
      <section id="selected-works" className="relative px-6 py-24 border-t border-gray-200">
        <div className={isFiltered ? "mb-8" : "mb-16"}>
        ...
        {/* ─── Filter bar (shown when URL has tags) ─── */}
        {isFiltered && (
          <div className="mb-10">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {filterTags.map((tag) => (
                <span
                  key={tag}
                  className="inline-block border border-black px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-wider"
                >
                  {TAG_TAXONOMY[tag] ?? tag}
                </span>
              ))}
              <a
                href="/"
                className="font-mono text-[10px] text-gray-400 hover:text-black uppercase tracking-wider ml-2 underline"
              >
                Show all works →
              </a>
            </div>
